...
The workspace itself is a tool to build user-specific pages. It comes with some standard workspaces which are listed in the `PUBLIC` section in the sidebar and are visible to all users. You can also create private workspaces which will only be visible to the logged-in user(owner) in the `MY WORKSPACES` section.
...
You can create as many private workspaces as you want. It will only be visible to the currently logged-in user(owner) under the `MY WORKSPACES` section
