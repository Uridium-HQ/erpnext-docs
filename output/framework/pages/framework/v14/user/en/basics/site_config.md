...
Imagine you want to enable the Frappe Web Logger on all the sites on your bench, except one _worker.frappe.xyz_ because let's assume it's a very high activity site and you just don't need to be flooded with a lot of information at once. You can set `enable_frappe_logger` as `true` in the `common_site_config.json` and set it to `false` in _worker.frappe.xyz_ 's `site_config.json`.
[code] 
...
`scheduler_tick_interval` | Job Scheduler runs a loop that enqueues all scheduled jobs pending execution once every _scheduler_tick_interval_ seconds. This makes it possible to set the minimum frequency for scheduling jobs. Defaults to 60.  
