...
[ Edit ](https://docs.frappe.io/wiki/spaces/r3uvq1ch61/page/12v3lue1tm)
...
> For references of any Bench commands, checkout [Bench Commands](bench-commands.md).
...
For more information and examples, see the [bench new-site](reference/new-site.md) reference.
...
For more information and examples, see the [bench backup](reference/backup.md) reference.
...
For more information and examples, see the [bench restore](reference/restore.md) reference.
...
For more information and examples, see the [bench partial-restore](reference/partial-restore.md) and [bench backup](reference/backup.md) reference.

#### Site App Management

You can install or uninstall Frappe Applications available on your Bench. To add Apps to your bench using `bench get-app` checkout [the docs](bench-commands.md).
...
For more information and examples, see the [bench uninstall-app](reference/uninstall-app.md) reference.

#### Site Migrations

Run patches, sync schema and rebuild files, translations and indexes on a particular site. Find out how site migrations work in [the docs](../database-migrations.md).
[code] 
...
For more information and examples, see the [bench migrate](reference/migrate.md) reference.

#### Site Deletion

Drops the database and moves the site directory from `./sites` to the `./archived_sites` _(unless specified otherwise)_ folder on your Bench.
[code] 
...
For more information and examples, see the [bench drop-site](reference/drop-site.md) reference.
...
For more information and examples, see the [bench reinstall](reference/reinstall.md) reference.
...
For more information and examples, see the [bench list-apps](reference/list-apps.md) reference.
...
Here are some operations you can perform on your site via the Bench CLI to update your site state. You may never have to use some of these, but they exist just in case.
  ...
  * **build-search-index** : Builds search index for Websites. Refer to [Full Text Search API Docs](../api/full-text-search.md) for more information.
  ...
  * **reload-doctype** : Reload schema for a particular _DocType_
...
For more information and examples, see the [bench transform-database](reference/transform-database.md) reference.
...
For more information and examples, see the [bench trim-tables](reference/trim-tables.md) reference.
...
For more information and examples, see the [bench trim-database](reference/trim-database.md) reference.
...
Commands to manage and review the scheduler and background jobs' statuses for the sites on your bench. Defined under module `frappe.commands.schedule`.
  ...
  * **scheduler** : Change the state of the scheduler for any site from _pause_ , _resume_ , _disable_ or _enable_ state.
...
The applied configuration for your sites gets applied as a combination of the bench directory's `common_site_config.json` and the site's own `site_config.json`. You can read more about this, [here](../basics/site_config.md). Bench provides an interface to view the applied `frappe.conf` values for your sites. You may choose to access this information in tabular or JSON formats.
[code] 
...
For more information and examples, see the [bench show-config](reference/show-config.md) reference.
...
For more information and examples, see the [bench set-config](reference/set-config.md) reference.
...
With version 13, the `-f` / `--format` option was introduced. This option allows you to display additional information about the branch and last commit. For more information and examples, see the [bench version](reference/bench-version.md) reference.
...
Set of commands to manage translation CSV-files for your multi-lingual deployments. These commands are defined under the module `frappe.commands.translate`.
...
Set of commands to manage translation gettext files for your multi-lingual apps. These commands are defined under the module `frappe.commands.gettext`.

  * **genereate-pot-file** : Extract all translatable strings and writes them into `[app]/locale/main.pot`
  * **migrate-csv-to-po** : Migrate existing translations from CSV files (old) to PO files (new)
  * **create-po-file** : Copy `main.pot` to create a PO file for a new locale
  * **update-po-files** : Update individual locale files with translatable strings from `main.pot`
  * **compile-po-to-mo** : Compile locale files to binary, optimized for fast retrieval



[ Previous Page Bench Commands  ](bench-commands.md) [ Next Page Bench - Extending the CLI  ](extending-the-cli.md)
